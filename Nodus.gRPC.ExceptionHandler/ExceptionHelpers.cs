    ...
    public static class ExceptionHelpers
    {
        public static RpcException Handle<T>(this Exception exception, ServerCallContext context, ILogger<T> logger, Guid correlationId) =>
            exception switch
            {
                ...
                BadRequestException => HandlBadRequestException((BadRequestException)exception, context, logger, correlationId),
        ...
        private static RpcException HandleNotFoundException<T>(NotFoundException exception, ServerCallContext context, ILogger<T> logger, Guid correlationId)
        {
            ...
            var status = new Status(StatusCode.NotFound, exception.Status.Detail);

            return new RpcException(status);
        }

        private static RpcException HandlBadRequestException<T>(BadRequestException exception, ServerCallContext context, ILogger<T> logger, Guid correlationId)
        {
            logger.LogError(exception, $"CorrelationId: {correlationId} - A timeout occurred");

            var status = new Status(exception.StatusCode, exception.Status.Detail);
